...
| **2. 思考占位** | T+0.5s~3s | T+2.5s（默认 `thinkingDelay`）出现"正在思考…"卡片或纯文本 | OpenCode 若 < 2.5s 内首事件返回，**占位卡片不会发**（race avoid） |
...
| 字段 | 默认 | 影响产品行为 |
|------|------|------------|
| `thinkingDelay` | 2500ms | 设为 0 关闭"正在思考…"占位卡片；用户在快回复场景看不到中间态，但慢回复（>3s）会看似卡顿 |
